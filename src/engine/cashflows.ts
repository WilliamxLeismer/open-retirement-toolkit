...
export function retirementNetNeedForMonth(scenario: Scenario, age: number, inflationFactor: number): number {
  ...
  return Math.max(0, spending - legacyIncome - namedIncome);
}

export function retirementWithdrawalForMonth(scenario: Scenario, age: number, inflationFactor: number): number {
  const netNeed = retirementNetNeedForMonth(scenario, age, inflationFactor);
  const taxDrag = scenario.effectiveTaxRate * scenario.taxableWithdrawalShare;
  return netNeed / Math.max(0.01, 1 - taxDrag);
}

export function oneTimeNetNeedForMonth(scenario: Scenario, month: number, inflationFactor: number): number {
  return scenario.oneTimeExpenses.reduce((total, expense) => {
    ...
    return total + expense.amount * (expense.inflationAdjusted ? inflationFactor : 1);
  }, 0);
}

export function oneTimeWithdrawalForMonth(scenario: Scenario, month: number, inflationFactor: number): number {
  const taxDrag = scenario.effectiveTaxRate * scenario.taxableWithdrawalShare;
  return oneTimeNetNeedForMonth(scenario, month, inflationFactor) / Math.max(0.01, 1 - taxDrag);
}
